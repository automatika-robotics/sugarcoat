"""The base of every action: dispatch, arguments, serialization, launch parsing.

Internal. Recipes import `Action` from `ros_sugar.core`, which adds the
monitoring machinery on top of this class. `BaseAction` exists as a separate
layer so that `event.py` can build on actions without importing the monitoring
side, which watches its success condition *as* an `Event` - importing it back
would be a cycle.
"""
...
class BaseAction:
    ...
    def deserialize_action(
        ...
        """Reconstruct the action from serialized action data
        ...
        # NOTE: cls (not BaseAction) so that subclasses such as Action
