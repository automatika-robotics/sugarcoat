...
Call an action `reset_buffer(size: int)` on `awesome_component`:

```shell
ros2 service call /awesome_component/execute_method automatika_ros_sugar/srv/ExecuteMethod "{name: 'reset_buffer', kwargs_json: '{\"size\": 10}'}"
```
